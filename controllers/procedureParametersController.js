...
class ProcedureParametersController {
    getProcedureTasks (req, res, next) {
        try {
            const targetProcedureTasks = procedureService.getFileFromDB(usersFile)
                .find(item => item.userId === Number(req.params.id)).data
                .find(item => item.id === Number(req.params.procedureId)).tasks;
