...
class ProcedureInfoController {
    editTaskSettings(req, res) {
        try {
            const newUserFile = procedureService.getFileFromDB(usersFile)
                .map(item => {
            ...
            const newProcedure = newUserFile.find(item => item.userId === Number(req.body.userId)).data.find(item => item.id === Number(req.body.procedureId));
